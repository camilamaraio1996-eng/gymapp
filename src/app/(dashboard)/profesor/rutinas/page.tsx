...
export default function ProfesorRutinasPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [{ data: r }, { data: a }] = await Promise.all([
      supabase.from('rutinas').select('*, ejercicios(*)').eq('created_by', user.id).order('created_at', { ascending: false }),
    ...
    <div className="flex flex-col gap-5 animate-fade-in">
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-[var(--muted-foreground)]">No creaste rutinas aún.</p>
              ...
              <CardContent className="p-0">
                ...
                {expandedId === rutina.id && rutina.ejercicios?.length ? (
                  <div className="border-t border-[var(--border)] px-4 pb-4">
                    <p className="text-xs font-semibold text-[var(--muted-foreground)] mt-3 mb-2 uppercase tracking-wider">{rutina.ejercicios.length} ejercicios</p>
                      ...
                      {rutina.ejercicios.map((ej, i) => (
                        <div key={ej.id} className="flex items-center gap-3 p-3 rounded-xl bg-[var(--accent)]">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-[var(--muted-foreground)]">{ej.series} series × {ej.repeticiones} reps{ej.descanso ? ` · ${ej.descanso}` : ''}</p>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <Select value={form.dias_por_semana} onValueChange={v => setForm(p => ({ ...p, dias_por_semana: v }))}>
                  ...
                  <SelectContent>
                    {[1, 2, 3, 4, 5, 6, 7].map(d => <SelectItem key={d} value={String(d)}>{d} días</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
            <div className="flex flex-col gap-1.5">
              ...
              <Input value={form.objetivo} onChange={e => setForm(p => ({ ...p, objetivo: e.target.value }))} placeholder="Ej: Hipertrofia" />
            </div>
            <div>
              <div className="flex items-center justify-between mb-3">
                <Label>Ejercicios</Label>
                <Button type="button" variant="outline" size="sm" onClick={() => setEjercicios(p => [...p, { ...emptyEjercicio }])}>
        ...
        <DialogContent>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Select value={selectedAlumno} onValueChange={setSelectedAlumno}>
              ...
              <SelectContent>
                {alumnos.length === 0 ? (
                  <div className="p-3 text-sm text-center text-[var(--muted-foreground)]">No tenés alumnos asignados</div>
